...
#define SELF_SQ1_START_THRESH  5000
	...
	int led_num)
{
	...
	//pr_info("%d-new_led_uA:%d,%d\n", led_num, new_led_uA, new_led_reg_val);
...
int __max869_hrm_agc(struct max869_device_data *data, int counts, int led_num)
{
	...
	if (err)
		return err;
	//pr_info("%d-a:%d\n", led_num, data->change_led_by_absolute_count[led_num]);
...
static void __max869_selftest_sequence_1(int ir_data, int red_data, struct  max86902_selftest_data *self_data)
{
	...
	if (!seq1_count) {
		HRM_dbg("EOL_TEST sequence 1 started !!");
	}
	...
	if (seq1_index < SELF_SQ1_SIZE) {
		self_data->seq1_data.buf[SELF_IR_CH][seq1_index] = (u64)ir_data*CONVER_FLOAT;
		self_data->seq1_data.buf[SELF_RED_CH][seq1_index] = (u64)red_data*CONVER_FLOAT;
...
static int __max86902_eol_test_control(struct max869_device_data *data)
{
	...
	} else if (data->sample_cnt < ((data->hr_range2 + 297) + 400)) {
	...
	} else if (data->sample_cnt < ((data->hr_range2 + 297) + 400 * 15)) {
	...
	if (device->eol_test_is_enable) {
		...
		if ((!device->selftest_data.status)) {
			...
			case _EOL_STATE_TYPE_END:
				...
				HRM_dbg("NERVER CALL !!!!FAIL EOL TEST  : HR_STATE: EOL_STATE_TYPE_END !!!!!!!!!!!!!!!!!!\n");
				__max869_selftest_check_done(&device->selftest_data, device->self_mode, device);
				break;
...
static int __max86902_awb_flicker_read_data(struct max869_device_data *device, int *data)
{
	...
	if (err != 0) {
		pr_err("%s __max869_read_reg err:%d, address:0x%02x\n",
			__func__, err, recvData[0]);
		return -EIO;
	}
