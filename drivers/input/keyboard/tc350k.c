...
#define KEY_TSP_TKEY_DUMMY	0
struct tsk_event_val tsk_ev_old[8] =
{
	{0x01, TSK_PRESS, KEY_BACK, "back"},
	{0x02, TSK_PRESS, KEY_RECENT, "recent"},
	{0x03, TSK_PRESS, KEY_TSP_TKEY_DUMMY, "dummy_back"},
	{0x04, TSK_PRESS, KEY_TSP_TKEY_DUMMY, "dummy_menu"},
	{0x09, TSK_RELEASE, KEY_BACK, "back"},
	{0x0A, TSK_RELEASE, KEY_RECENT, "recent"},
	{0x0B, TSK_RELEASE, KEY_TSP_TKEY_DUMMY, "dummy_back"},
	{0x0C, TSK_RELEASE, KEY_TSP_TKEY_DUMMY, "dummy_menu"}
};
...
static void tc300k_gpio_request(struct tc300k_data *data)
{
	...
	if (gpio_is_valid(data->pdata->en_gpio)) {
		ret = gpio_request(data->pdata->en_gpio, "touchkey_en");
		if (ret)
			dev_err(&data->client->dev, "%s: unable to request touchkey_en [%d]\n",
					__func__, data->pdata->en_gpio);
	}
}
			...
			struct tc300k_platform_data *pdata)
{
	...
	pdata->gpio_scl = of_get_named_gpio(np, "coreriver,scl-gpio", 0);
	pdata->gpio_sda = of_get_named_gpio(np, "coreriver,sda-gpio", 0);
	pdata->gpio_int = of_get_named_gpio(np, "coreriver,irq-gpio", 0);
	pdata->en_gpio = of_get_named_gpio(np, "coreriver,en_gpio", 0);

	pdata->boot_on_ldo = of_property_read_bool(np, "coreriver,boot-on-ldo");

	pdata->i2c_gpio = of_property_read_bool(np, "coreriver,i2c_gpio");
	...
	printk(KERN_INFO "[TK] %s: tkey_scl= %d, tkey_sda= %d, tkey_int= %d, tkey_sub_det= %d, en=%d, f/w=%s%s",
				__func__, pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int, pdata->gpio_sub_det,
				pdata->en_gpio, pdata->fw_name, pdata->boot_on_ldo ? " ,boot_on_ldo" : "");
...
int tc300k_touchkey_power(struct tc300k_data *data, bool on)
{
	...
	regulator = devm_regulator_get(&data->client->dev, "vtouch_1p8");
	if (IS_ERR(regulator)){
		pr_err("[TK] %s: regulator_ic get failed:%ld\n", __func__, PTR_ERR(regulator));
		return -EIO;
	}
	if (on) {
...
int tc300k_touchkey_led_control(struct tc300k_data *data, bool on)
{
	...
	regulator = devm_regulator_get(&data->client->dev, "vtouch_3p3");
	if (IS_ERR(regulator)){
...
static int tc300_pinctrl_configure(struct tc300k_data *data, bool active)
{
	...
	if (active) {
		set_state_i2c =
			pinctrl_lookup_state(data->pinctrl, "touchkey_active");
		...
		set_state_i2c =
			pinctrl_lookup_state(data->pinctrl,	"touchkey_suspend");
		if (IS_ERR(set_state_i2c)) {
...
//	data->sec_touchkey = sec_device_create(client, "sec_touchkey");
	...
	dev_info(&data->client->dev,
		"%s : LED_TWINKLE_BOOTING lcd_attached[%d], jig_status[%d]\n", __func__, get_lcd_attached("GET"), jig_status);

	if (get_lcd_attached("GET") == 0xFFFFFF && jig_status == false)
	{
		dev_info(&data->client->dev,
