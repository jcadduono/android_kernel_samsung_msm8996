 ...
 *
*/
#include "ss_dsi_smart_dimming_S6E3FA3_AMS549KU01.h"
...
static void gamma_init_revA(struct SMART_DIM *pSmart, char *str, int size)
{
	...
	*/
	for (cnt = 1; cnt < TABLE_MAX; cnt++) {
		...
		bl_index[TABLE_MAX - cnt] += gradation_offset_revA[table_index][cnt - 1];
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	...
	psmart->vregout_voltage = VREG0_REF_6P5;
...
struct smartdim_conf *smart_get_conf_S6E3FA3_AMS549KU01(void) {
...
struct smartdim_conf *smart_get_conf_S6E3FA3_AMS549KU01_hmt(void)
{
