 ...
 *
*/
#include "ss_dsi_panel_S6E3FA3_AMS549KU01.h"
#include "ss_dsi_mdnie_S6E3FA3_AMS549KU01.h"
...
static int mdss_cell_id_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (vdd->dtsi_data[ctrl->ndx].cell_id_rx_cmds[vdd->panel_revision].cmd_cnt) {
		...
		/* For AMS549KU01 exceptionally, MDNIE X&Y value(4bytes) should be get from A1h register read value */
...
static int mdss_elvss_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	/* Read mtp (B6h 22th) for elvss*/
	mdss_samsung_panel_data_read(ctrl,
			&(vdd->dtsi_data[ctrl->ndx].elvss_rx_cmds[vdd->panel_revision]),
			&elvss_b6_22th, PANEL_LEVE1_KEY);
	vdd->display_status_dsi[ctrl->ndx].elvss_value1 = elvss_b6_22th;
...
static int mdss_hbm_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (vdd->dtsi_data[ctrl->ndx].hbm_rx_cmds[vdd->panel_revision].cmd_cnt) {
		mdss_samsung_panel_data_read(ctrl,
			&(vdd->dtsi_data[ctrl->ndx].hbm_rx_cmds[vdd->panel_revision]), // B4 1th~4th
	...
	if (vdd->dtsi_data[ctrl->ndx].hbm2_rx_cmds[vdd->panel_revision].cmd_cnt) {
		mdss_samsung_panel_data_read(ctrl,
			&(vdd->dtsi_data[ctrl->ndx].hbm2_rx_cmds[vdd->panel_revision]), // B4 5th~33th
...
static char coordinate_data_2[][COORDINATE_DATA_SIZE] = {
	...
	{0xff, 0x00, 0xf9, 0x00, 0xec, 0x00}, /* Tune_4 */
	{0xff, 0x00, 0xfa, 0x00, 0xf0, 0x00}, /* Tune_5 */
	{0xff, 0x00, 0xfb, 0x00, 0xf5, 0x00}, /* Tune_6 */
	{0xff, 0x00, 0xfd, 0x00, 0xed, 0x00}, /* Tune_7 */
	{0xff, 0x00, 0xfe, 0x00, 0xf0, 0x00}, /* Tune_8 */
	{0xff, 0x00, 0xff, 0x00, 0xf5, 0x00}, /* Tune_9 */
};
...
static struct dsi_panel_cmds * mdss_acl_on(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	*level_key = PANEL_LEVE1_KEY;
	if (vdd->gradual_acl_val)
		vdd->dtsi_data[ctrl->ndx].acl_on_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = vdd->gradual_acl_val;
...
static struct dsi_panel_cmds * mdss_elvss_temperature2(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (vdd->temperature > 0) {
		...
		if (vdd->candela_level >= 5 && vdd->candela_level <= 20)
			elvss_22th_val = 0x1B;
		else if (vdd->candela_level == 21)
			elvss_22th_val = 0x19;
		else if (vdd->candela_level == 22)
			elvss_22th_val = 0x18;
		else if (vdd->candela_level == 24)
			elvss_22th_val = 0x17;
		else if (vdd->candela_level == 25)
			elvss_22th_val = 0x16;
		else if (vdd->candela_level == 27)
			elvss_22th_val = 0x15;
		else if (vdd->candela_level == 29)
			elvss_22th_val = 0x14;
		else if (vdd->candela_level == 30)
			elvss_22th_val = 0x13;
	} else {
		if (vdd->candela_level >= 5 && vdd->candela_level <= 20)
			elvss_22th_val = 0x1F;
	...
	vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = elvss_22th_val;
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3FA3_AMS549KU01;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3FA3_AMS549KU01_FHD";
